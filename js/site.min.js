function getRandomArrayElements(e, a) {
    for (var t, o, r = e.slice(0), n = e.length, i = n - a; n-- > i;) o = Math.floor((n + 1) * Math.random()), t = r[o], r[o] = r[n], r[n] = t;
    return r.slice(i)
}

function getReviews() {
    for (var e = 0; e < siteReviews.reviews.length; e++) reviewsArray.push(siteReviews.reviews[e]);
    if (reviewsArray.length > 0)
        for (var a = getRandomArrayElements(reviewsArray, 1), t = [], e = 0; e < a.length; e++) {
            t.push(a[e]);
            var o = a[e].review,
                r = a[e].source,
                n = a[e].link;
            document.getElementById("review").innerHTML = o, document.getElementById("review-link").innerHTML = r + " Review", document.getElementById("review-link").setAttribute("href", n)
        }
}
...
function createModal() {
    var e = document.createElement("div"),
        ...
        t = document.getElementById("hero-booking"),
        o = document.getElementById("footer-booking");
    (t || o) && (t.onclick = function (t) {
        e.id = "overlay", a.id = "modal", a.className = "visible", a.innerHTML = '<h1>Choose location</h1> <a style="margin-top:10px;" class="btn gradient" href="https://bookeo.com/gratitudebeauty">Brentwood</a> <a style="margin-top: 10px;" class="btn gradient" href="https://bookeo.com/gratitudebeauty-second">16th Ave NW</a>', document.body.appendChild(e), e.appendChild(a), t.preventDefault()
    }, o.onclick = function (t) {
        e.id = "overlay", a.id = "modal", a.className = "visible", a.innerHTML = '<h1>Choose location</h1> <a style="margin-top:10px;" class="btn gradient" href="https://bookeo.com/gratitudebeauty">Brentwood</a> <a style="margin-top: 10px;" class="btn gradient" href="https://bookeo.com/gratitudebeauty-second">16th Ave NW</a>', document.body.appendChild(e), e.appendChild(a), t.preventDefault()
    }), e.onclick = function () {
        this.parentElement.removeChild(e), this.removeChild(a)
...
var heroBgList = ["hero-1", "hero-2", "hero-3", "hero-4", "hero-5"],
    randomHeroBg = heroBgList[Math.floor(Math.random() * heroBgList.length)];
document.getElementById("hero").classList = randomHeroBg;
for (var imgs = document.getElementsByClassName("mobile"), i = 0; i < imgs.length; i++) {
    var num = Math.floor(5 * Math.random() + 1);
    imgs[i].src = "img/hero-" + num + "-sm.jpg", imgs[i].alt = imgs[i].src
}
var reviewsArray = [],
    siteReviews = {
        reviews: [{
